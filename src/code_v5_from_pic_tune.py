...
def train_dx_model(data_folder, model_folder, verbose):
    ...
    for _ in tqdm(range(n_epoch), desc="epoch", leave=False):
        ...
        #     # temp_data = mask_12channal_train(xtrain[i])
            ...
            mask_data = add_mask(noisy_data) # 添加缺失
        ...
        # plt.savefig("/data/0shared/zhangshanwei/cinc/ours/digitalization/train/picture/old_test/roc_"+str(count)+"epoch_v5.png")
        count = count + 1
        ...
        try:
            existing_df = pd.read_excel('../digitalization/train/table/random.xlsx')
        except FileNotFoundError:
            print("文件 'existing_file.xlsx' 不存在，将创建新文件。")
            df1 = pd.DataFrame([list_auc])
            df1.to_excel('../digitalization/train/table/random.xlsx', index=False, header=False)
            continue
        df2 = pd.DataFrame([list_auc])
        existing_df = pd.read_excel('../digitalization/train/table/random.xlsx', header=None)
        combined_df = pd.concat([existing_df, df2], ignore_index=True)
        combined_df.to_excel('../digitalization/train/table/random.xlsx', index=False, header=False) 
