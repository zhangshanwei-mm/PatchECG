...
class Dataset_ETT_minute(Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq_x = self.data_x[s_begin:s_end]
        seq_y = self.data_y[r_begin:r_end]
        seq_x_mark = self.data_stamp[s_begin:s_end] # mark 就是外源标记，应该是
        seq_y_mark = self.data_stamp[r_begin:r_end]
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        # print(df_raw.columns) # ['date', 'p (mbar)', 'T (degC)', 'Tpot (K)', 'Tdew (degC)', 'rh (%)', 'VPmax (mbar)', 'VPact (mbar)', 'VPdef (mbar)', 'sh (g/kg)', 'H2OC (mmol/mol)', 'rho (g/m**3)', 'wv (m/s)', 'max. wv (m/s)', 'wd (deg)', 'rain (mm)', 'raining (s)', 'SWDR (W/m�)', 'PAR (�mol/m�/s)', 'max. PAR (�mol/m�/s)', 'Tlog (degC)', 'OT']
        num_train = int(len(df_raw) * 0.7) # sample number
        ...
        df_stamp = df_raw[['date']][border1:border2]
        df_stamp['date'] = pd.to_datetime(df_stamp.date) # 2020-09-13 05:00:00...... 2020-09-13 05:10:00
        ...
        elif self.timeenc == 1: # 走的这边
            
            data_stamp = time_features(pd.to_datetime(df_stamp['date'].values), freq=self.freq) # 【36887】把时间转为这个
            # print(data_stamp[0])
            data_stamp = data_stamp.transpose(1, 0) # 4，36887 -》36887，4    |[HourOfDay, DayOfWeek, DayOfMonth, DayOfYear]
            print(data_stamp.shape) # 36887,4 
            print(data_stamp[0]) # [-0.5        -0.16666667 -0.5        -0.5       ]
            # print("=====")
            # exit()
        ...
        self.data_stamp = data_stamp # (n,4)
        print("data_stamp shapes is ",data_stamp.shape) # (n,4)
    ...
    def __getitem__(self, index):
        ...
        seq_x = self.data_x[s_begin:s_end]
        seq_y = self.data_y[r_begin:r_end]
        seq_x_mark = self.data_stamp[s_begin:s_end] # data_stamp 时间的时间戳
        seq_y_mark = self.data_stamp[r_begin:r_end] # mark 代表的就是普通的时间特征范围在[-0.5,0.5]之间
        return seq_x, seq_y, seq_x_mark, seq_y_mark
...
class my_dataLoader(Dataset):
    def __init__(self, data,label,args, root_path, flag=None):
        self.data = data
        self.label = label
        print("my_dataloader shape :",data.shape)
