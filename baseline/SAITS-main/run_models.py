...
def impute_all_missing_data(model, train_data, val_data, test_data):
        ...
        for dataloader, set_name in zip(
            ...
            for idx, data in enumerate(dataloader):
                if args.model_type in ["BRITS", "MRNN"]:
                    ...
                    indices, X, missing_mask = map(lambda x: x.to(args.device), data)
                    # print(map(lambda x: x.to(args.device), data).keys())
                    # print(X.shape) # [64,1000,12]
            ...
            # ours
            # label_collector = torch.cat(label_collector)
            # print(label_collector.shape)
            # print(label_collector[0])
    imputation_saving_path = os.path.join(args.result_saving_path, "imputations.h5")
    
    # add ptb-xl from pic label
    # with h5py.File('/data/0shared/zhangshanwei/data/2024cinc/from_picture_to_signal.h5', 'r') as f: # 原始的
    ...
    with h5py.File(imputation_saving_path, "w") as hf:
        ...
        # add ptb-xl label 
        # hf.create_dataset("labels", data=ytest_ndarray) # add from pic data
