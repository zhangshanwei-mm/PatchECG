...
def save_check(avg_auc, model1,model2,epoch,th):
    ...
    torch.save({
        ...
        'Best_thresholds':th
    }, os.path.join('/data/0shared/zhangshanwei/ECG_founder/ECGFounder-master/PatchEncoder/model/5fold/', 'checkpoint_'+str(epoch)+'.pth')) # 保存路径
    


def save_check_AF(result, model1,model2,epoch,th):
    print('Model Saving...')
    torch.save({
        'model1': model1.state_dict(),
        'model2': model2.state_dict(),
        'Result_test': result,
        'Best_thresholds':th
    }, os.path.join('/data/0shared/zhangshanwei/cinc/ours/src/AF/model/', 'checkpoint_'+str(epoch)+'.pth')) # 保存路径
...
if __name__ == "__main__":
    row_data = np.random.rand(1000, 12)
    
    a = gen_true_ecg_layout(row_data,length = 1000,layout = 0)
    b = gen_true_ecg_layout(row_data,length = 1000,layout = 1)
    c = gen_true_ecg_layout(row_data,length = 1000,layout = 2)
    d = gen_true_ecg_layout(row_data,length = 1000,layout = 3)
    e = gen_true_ecg_layout(row_data,length = 1000,layout = 4)
    
    
    np.savetxt("./test/1.csv", a, delimiter=',', fmt='%s')
    np.savetxt("./test/2.csv", b, delimiter=',', fmt='%s')
    np.savetxt("./test/3.csv", c, delimiter=',', fmt='%s')
    np.savetxt("./test/4.csv", d, delimiter=',', fmt='%s')
    np.savetxt("./test/5.csv", e, delimiter=',', fmt='%s')
