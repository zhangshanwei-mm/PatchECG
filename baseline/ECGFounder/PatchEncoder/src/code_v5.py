...
def train_dx_model(data_folder, model_folder, verbose):
    ...
    model1 = Net1D(
        ...
        use_do=False,
        n_classes=768)
    checkpoint = torch.load("/data/0shared/zhangshanwei/ECG_founder/ECGFounder-master/checkpoint/1_lead_ECGFounder.pth", map_location=device)
    state_dict = checkpoint['state_dict']

    state_dict = {k: v for k, v in state_dict.items() if not k.startswith('dense.')} 

    model1.load_state_dict(state_dict, strict=False)# 移除最后一层

    model1.dense = nn.Linear(model1.dense.in_features, 768).to(device) # 替换最后一层
    # freezing model
    linear_prob = True
    ...
    for _ in tqdm(range(n_epoch), desc="epoch", leave=False):
        ...
        for batch_idx, batch in enumerate(prog_iter):
            ...
            # temp_x [B x 12 x15,64]
            PatchEncode_output = model1(temp_x.unsqueeze(1)) # [B x 12 x15,1,768]

            # print(PatchEncode_output.shape) # [B x 12 x15,768]
            output_x = PatchEncode_output.view(input_x.shape[0],input_x.shape[1],input_x.shape[2],768)

            # print(output_x.shape)[64, 12, 15, 768]
            output_x[mask] = 0
        ...
        plt.savefig("/data/0shared/zhangshanwei/ECG_founder/ECGFounder-master/PatchEncoder/picture/train/5fold/roc_"+str(count)+"epoch_v5.png")
        count = count + 1
        ...
        # save table path
        save_table_path = '/data/0shared/zhangshanwei/ECG_founder/ECGFounder-master/PatchEncoder/table/train/5fold.xlsx'
    ...
    # plt.legend()
    # plt.savefig("/data/0shared/zhangshanwei/ECG_founder/ECGFounder-master/PatchEncoder/picture/1fold_v5.png")  
